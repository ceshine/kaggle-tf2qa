...
def get_data(tokenizer, pattern, max_q_len, max_ex_len, batch_size):
...
def main(
    pattern: str = "cache/train_*.jl", max_q_len: int = 128,
    max_ex_len: int = 350, batch_size: int = 4, n_steps: int = 10000,
    lr: float = 3e-4, grad_accu: int = 1
):
    tokenizer = BertTokenizer.from_pretrained(MODEL_NAME)
    train_loader, valid_loader = get_data(
        tokenizer, pattern, max_q_len, max_ex_len, batch_size)
